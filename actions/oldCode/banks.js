...
export const getBankAccountsss = async () => {
  try {
    // 1. Check if user exists and is logged in
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized User!");
...
export const getCacheBankAccountss = async (page = 1) => {
  try {
...
export const editBankAccount = async (id, data) => {
  try {
    // 1. Check if user exists and is logged in
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized User!");
...
// This is used to add a bank account
